"""
FLUX 图像编辑水印批处理（对齐 SD3_watermark/image_edit.py）
...
单张 demo:
  python FLUX_watermark/image_edit.py --demo --img_in xxx.jpg --edit_prompt "..."
"""
import argparse
import os
import sys
...
from watermarker import FluxImgEditWatermarker
from utils import (
    TRADITIONAL_ATTACKS,
    bit_accuracy,
    build_vae_networks,
...
def run_demo(model_path, img_in, message, edit_prompt, strength, denoising_strength, save_path):
    marker = FluxImgEditWatermarker(model_path, strength=strength, num_chars=len(message))
    print("\n[demo] FLUX 编辑嵌入水印...")
...
def parse_args():
    p = argparse.ArgumentParser(description="FLUX image-edit watermark evaluation")
    p.add_argument("--demo", action="store_true", help="单张编辑 demo")
    p.add_argument("--img_in", default="000000001584.jpg")
    p.add_argument(
        "--edit_prompt",
        default="a fire engine on the road, high quality, detailed, photorealistic",
    )
    p.add_argument("--demo_out", default="edit_flux.png")

    p.add_argument("--output_dir", default="./output_flux_edit")
    ...
    p.add_argument(
        "--prompts_csv",
        default="../../llm_model/W-Bench/DET_INVERSION_1K/prompts.csv",
    )
    p.add_argument(
        "--image_dir", default="../../llm_model/W-Bench/DET_INVERSION_1K/image"
    )
