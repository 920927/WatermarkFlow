"""
FLUX 图生图水印批处理（对齐 SD3_watermark/image_to_image.py）
...
默认参数对齐 SD3 i2i 的质量/鲁棒性平衡：
  - denoising_strength=0.06（低改图，PSNR ~34dB）
  - 注入窗口改为晚期 50<t<450（使低 ds 仍有注入步）
  - strength=0.017, extract_threshold=0.008
"""
import argparse
import os
import sys
...
from watermarker import FluxImg2ImgWatermarker, _FluxWatermarkBase
from utils import (
    TRADITIONAL_ATTACKS,
    bit_accuracy,
    build_vae_networks,
...
# FLUX 调度在低 ds 时落在晚期 t；默认 200<t<600 会空窗。
# 仅在本脚本运行时把注入窗口调到 50<t<450，不改 watermarker.py。
_ORIG_MAKE_CALLBACK = _FluxWatermarkBase._make_callback


def _make_callback_late(self, bits, t_lo=50, t_hi=450):
    return _ORIG_MAKE_CALLBACK(self, bits, t_lo=t_lo, t_hi=t_hi)


_FluxWatermarkBase._make_callback = _make_callback_late
...
def batch_process(
    ...
    print(
        f"[*] images={len(image_files)}, metric={metric}, attacks={attacks}, "
        f"strength={strength}, ds={denoising_strength}, thr={extract_threshold}, "
        f"inject_window=50<t<450"
    )
...
def parse_args():
    p = argparse.ArgumentParser(description="FLUX image-to-image watermark evaluation")
    p.add_argument("--input_dir", default="../dataset/val2017")
    p.add_argument("--output_dir", default="./output_flux_i2i")
    ...
    p.add_argument("--strength", type=float, default=0.017)
    p.add_argument("--denoising_strength", type=float, default=0.06)
    p.add_argument("--extract_threshold", type=float, default=0.008)
